.PHONY: build build-release build-ebpf build-agent build-ui build-ml run run-release clean check clippy fmt test help
...
build-agent: ## Build only the userspace agent
...
build-all-features: ## Build the agent with all optional features
	cargo build -p busted-agent --features ml,k8s
...
run-release: build-release ## Build and run the agent in release mode
	sudo ./target/release/busted
...
run-ui: build-ui ## Run the dashboard UI
...
check: ## Type-check all crates (no codegen)
	...
	cargo check -p busted-agent --features ml
	cargo check -p busted-agent --features k8s

clippy: ## Run clippy lints
	cargo clippy --workspace -- -D warnings
	cargo clippy -p busted-agent --features ml -- -D warnings
